...
import { fetchFixturesBySeason } from '@/lib/api-football'

export async function GET(request) {
  const { searchParams } = new URL(request.url)
  const season = parseInt(searchParams.get('season') ?? '')
  if (!season || season < 2020 || season > 2025) {
    return Response.json({ ok: false, error: 'season パラメータが不正です（2020〜2025）' }, { status: 400 })
  }

  try {
    const response = await fetchFixturesBySeason(season)
    let upserted = 0
    ...
    return Response.json({ ok: true, season, upserted })
  } catch (err) {
    console.error(err)
