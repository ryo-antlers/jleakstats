...
export async function GET() {
  ...
  if (hasConfirmed) {
    // 確定ポイントをfantasy_pointsから取得
    const pts = await sql`
      SELECT player_id, SUM(points) AS points
      FROM fantasy_points WHERE gameweek_id = ${lastGw.id}
      GROUP BY player_id
    `
    pointsMap = Object.fromEntries(pts.map(p => [p.player_id, Number(p.points)]))
  } else {
    // fixture_player_statsから暫定計算
  ...
  const result = squadRows.map(p => ({
    ...
    points: playedPlayerIds.has(p.player_id) ? (pointsMap[p.player_id] ?? 0) : 0,
  }))
