...
export async function POST(request) {
  ...
  if (displayName.length < 1 || displayName.length > 30) {
    return Response.json({ error: '表示名は1〜30文字で入力してください' }, { status: 400 })
  }
  if (containsNG(displayName)) {
    return Response.json({ error: '表示名に使用できない言葉が含まれています' }, { status: 400 })
  }

  // supported_club_id のバリデーション
  ...
  const clubRows = await sql`
    ...
    WHERE id = ${supportedClubId} AND group_name IN ('EAST', 'WEST')
  `
  ...
  if (existing.length > 0) {
    ...
    await sql`
      INSERT INTO user_profiles (
        clerk_user_id, display_name, supported_club_id,
        club_changed_at, created_at, updated_at
      ) VALUES (
        ${userId}, ${displayName}, ${supportedClubId},
        NOW(), NOW(), NOW()
      )
    `
  }
