        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', alignItems: 'start', paddingTop: 16, paddingBottom: 12, gap: 16 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8 }}>
            ...
            <div style={{ textAlign: 'right' }}>
              <span style={{ fontSize: 22, fontWeight: 700, color: squad.length >= 15 ? 'var(--accent)' : 'var(--text-secondary)' }}>{squad.length}</span>
              <span style={{ fontSize: 15, color: 'var(--text-secondary)', fontWeight: 400 }}>/15</span>
            </div>
          </div>
        </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 20, flexShrink: 0 }}>
                    ...
                    {String(actionLoading) === String(p.id) ? (
                      ...
                      <button
                        ...
                        title={!canAdd ? (
                          clubOver ? `${p.team_abbr}の選手を既に3人選んでいます` :
                          budgetOver ? '資金が不足しています' :
                          posOver ? `${p.position}の上限に達しています` :
                          squadFull ? 'スカッドが上限（20人）に達しています' : ''
                        ) : undefined}
                        style={{
