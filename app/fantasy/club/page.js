...
export default function ClubPage() {
  ...
  useEffect(() => {
    fetch('/api/fantasy/me')
      .then(r => r.json())
      .then(d => {
        if (!d.user) { router.push('/fantasy/setup'); return }
        setUsername(d.user.username ?? '')
        setTeamName(d.user.team_name ?? '')
        setTeamColor(d.user.team_color ?? '#00ff87')
      })
      .catch(() => router.push('/fantasy/setup'))
  ...
  return (
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: 400, textAlign: 'center' }}>
        ...
        <h1 style={{ fontSize: 28, fontWeight: 700, marginBottom: 16, whiteSpace: 'nowrap', overflow: 'visible' }}>
          <span style={{ color: teamColor }}>{teamName || 'クラブ情報'}</span>
        </h1>

        <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 32, lineHeight: 1.8 }}>
          監督名・クラブ名・クラブカラーを変更できます。<br />
          選手やポイントはそのまま引き継がれます。
        </p>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 28, textAlign: 'left' }}>
          ...
          {success && (
            <p style={{ fontSize: 12, color: 'var(--accent)', textAlign: 'center' }}>保存しました</p>
          )}
