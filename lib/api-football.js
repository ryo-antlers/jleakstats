...
async function apiFetch(endpoint) {
  ...
  // API-Football は HTTP 200 で返しつつ data.errors にプラン制限やレート超過を入れてくる
  // ことがある。silently 空配列で返さず、明示的に例外化する。
  if (data.errors) {
    const errs = Array.isArray(data.errors) ? data.errors : Object.entries(data.errors)
    if (errs.length > 0) {
      const msg = errs.map((e) => (Array.isArray(e) ? `${e[0]}: ${e[1]}` : e)).join(' | ')
      throw new Error(`API-FOOTBALL response error (${endpoint}): ${msg}`)
    }
  }
...
// 指定した fixture ID 群の現状を取得 (live 終了直後の状態確認に使う)
export async function fetchFixturesByIds(ids) {
  if (!ids?.length) return []
  // API-Football の ids パラメータは `-` 区切り、最大 20 件
  const chunk = ids.slice(0, 20).join('-')
  return apiFetch(`/fixtures?ids=${chunk}`)
}
...
export async function fetchStandings(league = API_LEAGUE_J1) {
  return apiFetch(`/standings?league=${league}&season=${SEASON}`)
}

export async function fetchPlayersByTeam(teamId, season = SEASON, page = 1) {
  return apiFetch(`/players?team=${teamId}&season=${season}&page=${page}`)
}

// 個別 player 詳細取得 (canonical 紐付け確認等)
export async function fetchPlayerById(playerId, season = SEASON) {
  return apiFetch(`/players?id=${playerId}&season=${season}`)
}

// 指定リーグ × シーズンのチーム一覧 (squad 巡回用)
export async function fetchTeamsByLeague(leagueId, season) {
  return apiFetch(`/teams?league=${leagueId}&season=${season}`)
}

// API-Football の rate-limit (10 req/sec 制限) を回避するための簡易スリープ
