...
/**
 * FANTYPE の clubId (例: "omiya") と jleakstats teams_master.name_ja の例外マッピング。
...
/**
 * server-only: DB から FANTYPE の上書きデータをまとめて読み込む。
 ...
 *   }
 */
export async function loadFantypeState() {
  const [vectorRows, questionRows, teamRows] = await Promise.all([
    sql`SELECT club_id, axis_id, value FROM fantype_vector_overrides`.catch(() => []),
    sql`SELECT club_id, question_id, value FROM fantype_question_overrides`.catch(() => []),
  ...
  // teams_master.name_ja → id のマップを作り、FANTYPE clubId → teamId に変換
