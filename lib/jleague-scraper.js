...
export async function fetchRoundListing(jleaguePath) {
  const url = `https://www.jleague.jp/match/${jleaguePath}/2026/`
  ...
  $(`a[href*="/match/${jleaguePath}/2026/"]`).each((_, a) => {
    const href = $(a).attr('href')
    const m = href?.match(new RegExp(`/match/${jleaguePath}/2026/(\\d{6})/`))
    if (!m) return
    const code = m[1]
...
export async function fetchAjaxLiveHtml(jleaguePath, matchCode) {
  const targetPath = `/match/${jleaguePath}/2026/${matchCode}/`
